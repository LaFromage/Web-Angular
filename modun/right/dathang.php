...
if(isset($_POST['btdathang'])){
	...
	if($hoten==""||$Sdt==""||$diachi==""){
		...
		if(!is_numeric($Sdt)){
			...
			for($i=0;$i<$dem;$i++){
				$sql_sanpham="select*from chitietsanpham where id_sp='{$_SESSION['giohang'][$i]['id']}'";
				$query_sanpham=mysqli_query($connect,$sql_sanpham);
				$row_sanpham=mysqli_fetch_assoc($query_sanpham);
				$masp=$row_sanpham['id_sp'];
				$gia=$row_sanpham['gia'];
				$sql_chitietgiohang="insert into chitietgiohang (id_giohang,id_sp,soluong,gia) 	VALUES('{$idgiohang}','{$masp}','{$_SESSION['giohang'][$i]['soluong']}','{$gia}')";
